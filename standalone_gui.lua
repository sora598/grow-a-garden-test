-- Grow a Garden - Fully Self-Contained GUI
-- No external dependencies - everything included locally
...
print("✓ Starting Grow a Garden GUI...")

-- ========== LOCAL LUCK BOOST SYSTEM ==========
local LuckBoost = {}

function LuckBoost.apply(base, boostPercent)
    local factor = 1 + (boostPercent / 100)
    return base * factor
end

function LuckBoost.applyCap(base, boostPercent, minLuck, maxLuck)
    local result = LuckBoost.apply(base, boostPercent)
    if type(minLuck) == "number" and result < minLuck then
        result = minLuck
    end
    if type(maxLuck) == "number" and result > maxLuck then
        result = maxLuck
    end
    return result
end
...
-- ========== LOCAL LUCK CALCULATION ==========
local function getEffectiveLuck(config)
    local base = tonumber(config and config["LuckBase"]) or 0.0
    local boost = tonumber(config and config["LuckBoostPercent"]) or 0.0
    local minLuck = tonumber(config and config["LuckCapMin"])
    local maxLuck = tonumber(config and config["LuckCapMax"])
...
createToggle("Auto Water Plants", "Auto Water Plants", false, 3)
createToggle("Auto Favorite Items", "Auto Favorite Backpack", false, 4)

createSection("🍀 Luck Boost", 5)
createSlider("Base Luck", "LuckBase", 0, 1, 0.10, "", 6)
createSlider("Boost Percentage", "LuckBoostPercent", 0, 500, 50, "%", 7)
createSlider("Max Cap", "LuckCapMax", 0, 1, 0.80, "", 8)

createSection("⏱️ Timing", 9)
createSlider("Collect Delay", "Delay To Collect", 0.01, 1, 0.05, "s", 10)
createSlider("Water Delay", "DelayToWait", 0.01, 1, 0.1, "s", 11)

createSection("📊 Status", 12)
local statusLabel = createStatusBox(13)
local startBtn = createStartButton(14)

-- ========== FUNCTIONALITY ==========
local function updateStatus()
    local luck = getEffectiveLuck(config)
    ...
    if running then
        ...
        task.spawn(function()
            while running do
                ...
                if config["Auto Collect Fruits"] then
                    stats.itemsCollected = stats.itemsCollected + 1
                end
                if config["Auto Water Plants"] then
                    stats.plantsWatered = stats.plantsWatered + 1
                end
                task.wait(1)
            end
        end)
...
print("✓ All systems are self-contained")
updateStatus()
